...
### nvim-lspconfigで言語サーバーを設定する

言語サーバーをインストールしたら、`plugin/40_plugins.lua` にLSPの設定を追記する：

```lua
-- plugin/40_plugins.lua（抜粋）
...
```vim
<Leader>rn  " リネーム（Rename）
<Leader>ca  " コードアクション（Code Action）
```
...
```vim
" JavaScriptファイルで
log<Tab>   " console.log() が展開される
fun<Tab>   " function が展開される
arr<Tab>   " array関連のスニペットが展開される
```
...
```vim
<Leader>cf  " 現在のバッファをフォーマット（デフォルトキーマップ）
```
...
// 5. <Leader>rn でリネーム
getUserById  // ← <Leader>rn で全ファイルのgentByIdを一括リネーム
```
...
2. `nvim-lspconfig` で設定を書く
3. `gd`, `gr`, `K`, `<Leader>rn` などのキーマップを覚える
